...
public class SoundManage implements SharedPreferences.OnSharedPreferenceChangeListener {

    private final SoundPool mSounds;

    private final Map<Integer,Integer> mSoundIds = new HashMap<>();

    private final int [] soundFiles; // 音效

    private final float [] soundVolumes;
    private final String [] soundUses;
    ...
    private final int[] mBGMSongIds; // 背景音乐
    private MediaPlayer mBGMPlayer;
    private float mBGMVolume;


    public SoundManage(final Context context) {
        mContext = context;
    ...
    private void play(int soundKey, float speed, int loop) {
        ...
        try {
            if (sfvolume>0 && isReady() && !mMute && appPreferences.getBoolean("use_sound_effects", true)) {
            ...
            Log.e("SoundManage", "Error playing " + soundKey, e);
        }
    }
